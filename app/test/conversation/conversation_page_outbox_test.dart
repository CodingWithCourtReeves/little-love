...
void main() {
  ...
  testWidgets('every message in a sent run gets its own heart — no collapsing',
      (tester) async {
    final container = _container();
    addTearDown(container.dispose);
    await container.read(accountProvider.future);
  ...
  testWidgets('a failed run shows one caption below and tap retries the stack',
      (tester) async {
    final retried = <String>[];
    final container = _container();
    addTearDown(container.dispose);
